...
class Minimax(
     ...
     */
    fun minimax(board: Board, currDepth: Int, isMaximizingPlayer: Boolean, alpha: Int, beta: Int, player: Player, currHash: Long): Int {
        var alpha = alpha
        var beta = beta
        var bestValue: Int
        if (isMaximizingPlayer) {
            ...
            for (currentMove in possibleMoves) {
                ...
                value = if (winnerFinder.isMoveLeadToWin(board, currentMove)) {
                    ...
                    minimax(board, currDepth + 1, false, alpha, beta, getRival(player), newHash)
                }
            ...
            for (currentMove in possibleMoves) {
                ...
                value = if (winnerFinder.isMoveLeadToWin(board, currentMove)) {
                    ...
                    minimax(board, currDepth + 1, true, alpha, beta, getRival(player), newHash)
                }
                board.updateCellValue(currentMove.i, currentMove.j, CellType.EMPTY)
